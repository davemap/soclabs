export const referenceDesigns = [
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "controller", external: true, techId: "standard-peripherals", gateCount: "~2K", info: "ARM CoreSight-compatible Serial Wire Debug (SWD) interface providing non-intrusive debug access. Supports breakpoints, watchpoints, and memory access via an external debug probe (e.g. CMSIS-DAP).", configOptions: ["SWD only (default)", "SWD + JTAG (optional, +0.5K gates)"] },
      { name: "Expansion Subsystem", type: "subsystem", icon: "interface", techId: "hw-acceleration", gateCount: "~0.7K", info: "An expandable region on the AHB-Lite bus providing SRAM storage and a slot for custom accelerators. Users can attach their own IP blocks here without modifying the core interconnect.", subBlocks: [
        ...
        { name: "Custom Accelerator", type: "interface", techId: "hw-acceleration", gateCount: "User-defined", info: "A user-defined hardware accelerator slot connected via the AHB-Lite slave interface. This is where students and researchers integrate their own custom IP — such as DSP filters, ML inference engines, or crypto blocks." }
      ] },
    ],
  ...
  },
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "controller", external: true, techId: "standard-peripherals", gateCount: "~6K", info: "ARM CoreSight debug subsystem with SWD/JTAG interface, ETM trace, and ITM stimulus ports. Provides full non-intrusive debug with hardware breakpoints, data watchpoints, and real-time trace via TPIU.", configOptions: ["SWD + 4 breakpoints (default)", "SWD + JTAG + ETM trace (full, +3K gates)"] },
  ...
  },
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "controller", external: true, techId: "standard-peripherals", gateCount: "~10K", info: "ARM CoreSight debug and trace subsystem with JTAG/SWD access, ETM program trace, and cross-trigger interface (CTI). Supports multi-core debug synchronisation and real-time trace export.", configOptions: ["SWD + ETM (default)", "Full CoreSight with CTI + TPIU (optional, +5K gates)"] },
