export const referenceDesigns = [
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "debug", external: true, techId: "debug", gateCount: "~2K", info: "ARM CoreSight-compatible Serial Wire Debug (SWD) interface providing non-intrusive debug access. Supports breakpoints, watchpoints, and memory access via an external debug probe (e.g. CMSIS-DAP).", configOptions: ["SWD only (default)", "SWD + JTAG (optional, +0.5K gates)"] },
    ...
    moduleHierarchy: [
      ...
      { name: "Chip Core", type: "group", description: "Top-level chip module (nanosoc_chip.v). Instantiates clock/reset wiring, SWD pad routing, GPIO pad-to-port mapping, and the SoC core. Targeted for FPGA synthesis or ASIC tapeout.", techId: "system-integration", children: [
        { name: "SoC System", type: "group", description: "The SoC system integration level (nanosoc_system.v). Instantiates all subsystems, memory regions, bus matrix, and performs interrupt/DMA/IO wiring. Defines the complete memory map and interrupt topology.", children: [
          { name: "CPU Subsystem", type: "group", description: "CPU Subsystem (u_ss_cpu). Contains the ARM Cortex-M0 processor with clock/reset generation (PORESETn, HCLK, HRESETn), SWD debug port, and AHB-Lite master interface. Also routes boot ROM, IMEM, and DMEM AHB slave ports.", techId: "arm-cortex-m0", children: [
            { name: "Cortex-M0 Core", type: "module", description: "ARM Cortex-M0 processor core — ~12K gates, 0.9 DMIPS/MHz, 3-stage pipeline, single-cycle multiplier. Configurable: NUMIRQ=32, BKPT=4, WPT=2, WIC support.", techId: "arm-cortex-m0" },
            { name: "Clock & Reset Controller", type: "module", description: "Clock and reset controller — generates SYS_FCLK, SYS_HCLK, SYS_HRESETn, SYS_PORESETn from external clock and reset inputs. Handles SYSRESETREQ, debug reset, and watchdog reset." },
            { name: "Boot ROM", type: "module", description: "Boot ROM region — parameterised address width (default 2KB). Holds initial boot sequence and vector table at reset vector address.", techId: "memory-controllers" },
          ...
          { name: "Debug Subsystem", type: "group", description: "Debug Subsystem (u_ss_debug). SoC-level debug access via ADP (ARM Debug Processor) with AHB-Lite master for memory access, GPIO interface (GPO8/GPI8), and AXI-Stream byte interfaces for ADP/STDIN/STDOUT.", techId: "debug", children: [
            { name: "ADP Controller", type: "module", description: "ARM Debug Processor — provides non-intrusive AHB master debug access via command-line interface. Supports memory read/write, system reset, and GPIO control.", techId: "debug" },
            { name: "Serial Debug Controller", type: "module", description: "USRT Controller — APB-mapped serial controller bridging ADP STDIN/STDOUT streams. Configurable baud rate divider, TX/RX FIFOs, interrupt support.", techId: "debug" },
            { name: "USB Bridge Controller", type: "module", description: "FT1248 Controller — FTDI FT1248 serial interface for host PC communication via USB. Supports 1/2/4-bit width, configurable clock divider.", techId: "debug" },
          ]},
          { name: "System Control", type: "group", description: "System Control Subsystem (u_ss_systemctrl). Contains the APB peripheral bus, AHB-to-APB bridge, GPIO controllers, timer, watchdog, UART, and system ROM table. Generates APB clocks (PCLK, PCLKG) and peripheral resets.", techId: "standard-peripherals", children: [
            ...
            { name: "GPIO Port 1", type: "module", description: "16-bit GPIO with output mux for FT1248/EXTIO mode switching. Lower pins conditionally routed to FT1248 or EXTIO8x4 interface.", techId: "standard-peripherals" },
            ...
            { name: "Watchdog", type: "module", description: "32-bit watchdog with lock register. Generates reset if firmware fails to service within timeout.", techId: "standard-peripherals" },
            { name: "System ROM Table", type: "module", description: "CoreSight ROM table at 0xF0000000 with SoC identification: JEPID=0x51 (SoCLabs), part number, revision.", techId: "standard-peripherals" },
          ]},
          { name: "Interconnect", type: "group", description: "Interconnect Subsystem (u_ss_interconnect). Multi-master AHB bus matrix connecting 3 masters (CPU, DMAC_0, DMAC_1, Debug) to 8 slave regions. Address decoder with REMAP support for boot remapping.", techId: "ahb-lite", children: [
            { name: "Bus Matrix", type: "module", description: "AHB bus matrix — arbitrates between CPU, DMA Controller 0, DMA Controller 1, and Debug master ports. Routes to Boot ROM, Instruction Memory, Data Memory, System I/O, and Expansion slave regions.", techId: "ahb-lite" },
            { name: "Address Decoder", type: "module", description: "Parameterised address decoder with REMAP support. REMAP bit swaps Boot ROM and Instruction Memory mapping for post-boot operation.", techId: "ahb-lite" },
            { name: "Default Slave", type: "module", description: "Returns error response for accesses to unmapped address regions, preventing bus hangs.", techId: "ahb-lite" },
          ]},
          { name: "Expansion Subsystem", type: "group", description: "Expansion Subsystem (u_ss_expansion). User-extensible region with AHB slave port, two SRAM banks (low + high), and DMA/interrupt connections. Optionally supports DMA-350 AXI-Stream channels.", techId: "hw-acceleration", children: [
            { name: "Expansion SRAM Low", type: "module", description: "Lower expansion SRAM — default 16KB single-cycle SRAM. Serves as scratchpad or additional program memory for accelerators.", techId: "memory-controllers" },
  ...
  },
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "debug", external: true, techId: "debug", gateCount: "~6K", info: "ARM CoreSight debug subsystem with SWD/JTAG interface, ETM trace, and ITM stimulus ports. Provides full non-intrusive debug with hardware breakpoints, data watchpoints, and real-time trace via TPIU.", configOptions: ["SWD + 4 breakpoints (default)", "SWD + JTAG + ETM trace (full, +3K gates)"] },
  ...
  },
  {
    ...
    blockDiagram: [
      ...
      { name: "Debug Controller", type: "debug", external: true, techId: "debug", gateCount: "~10K", info: "ARM CoreSight debug and trace subsystem with JTAG/SWD access, ETM program trace, and cross-trigger interface (CTI). Supports multi-core debug synchronisation and real-time trace export.", configOptions: ["SWD + ETM (default)", "Full CoreSight with CTI + TPIU (optional, +5K gates)"] },
