export const referenceDesigns = [
  {
    ...
    blockDiagram: [
      { name: "ARM Cortex-M0", type: "processor", info: "The ARM Cortex-M0 is the smallest ARM processor, optimised for ultra-low gate count and minimal power. In NanoSoC it runs at up to 50 MHz with a single-cycle multiplier, providing a 0.9 DMIPS/MHz performance profile ideal for embedded control tasks." },
      { name: "AHB-Lite Bus", type: "interconnect", info: "A single-master AHB-Lite interconnect with a simple address decoder routing transactions to peripheral slaves. Supports 32-bit data transfers with single-cycle slave access for tightly-coupled peripherals." },
      { name: "SRAM (32 KB)", type: "memory", info: "32 KB of single-cycle SRAM providing code and data storage. Mapped at the base of the address space for zero-wait-state access from the Cortex-M0. Sufficient for typical embedded firmware images." },
      { name: "DMA Controller", type: "controller", info: "A lightweight 2-channel DMA controller enabling autonomous data transfers between peripherals and memory without CPU intervention. Supports byte, half-word, and word transfers with configurable priority levels." },
      { name: "GPIO", type: "peripheral", external: true, info: "16-bit general-purpose I/O port with configurable direction, pull-up/pull-down resistors, and interrupt generation on rising/falling edges. directly to FPGA/ASIC pads for external connectivity." },
      { name: "UART", type: "peripheral", external: true, info: "Full-duplex UART with configurable baud rate (up to 3 Mbaud), 8N1/8E1 framing, and 16-byte TX/RX FIFOs. Serves as the primary serial debug and communication interface to external hosts." },
      { name: "Timer", type: "peripheral", info: "32-bit dual-channel timer/counter with prescaler, auto-reload, and interrupt generation. Supports periodic timebase generation and input capture for timing measurements." },
      { name: "Watchdog", type: "peripheral", info: "32-bit watchdog timer with lock register to prevent accidental disable. Generates system reset if not serviced within the programmed timeout window, ensuring firmware liveness." },
      { name: "Debug Controller", type: "controller", external: true, info: "ARM CoreSight-compatible Serial Wire Debug (SWD) interface providing non-intrusive debug access. Supports breakpoints, watchpoints, and memory access via an external debug probe (e.g. CMSIS-DAP)." },
      { name: "Extension Port", type: "interface", info: "An exposed AHB-Lite slave port allowing users to attach custom accelerators or additional peripherals. Provides address, data, and control signals with a standardised handshake for easy IP integration." },
    ],
  ...
  },
  {
    ...
    blockDiagram: [
      { name: "ARM Cortex-M3", type: "processor", info: "The ARM Cortex-M3 processor with hardware multiply/divide and Thumb-2 ISA, running at up to 100 MHz. Provides 1.25 DMIPS/MHz with a 3-stage pipeline, MPU, and bit-banding for atomic peripheral access." },
      { name: "AHB Bus Matrix", type: "interconnect", info: "Multi-layer AHB bus matrix enabling concurrent master access. Supports simultaneous transactions from the processor and DMA controller to different slaves, significantly improving system throughput." },
      { name: "SRAM (128 KB)", type: "memory", info: "128 KB of zero-wait-state SRAM split into two banks for concurrent access by the processor and DMA controller. Provides ample space for stack, heap, and data buffers." },
      { name: "Flash (256 KB)", type: "memory", info: "256 KB of embedded flash memory for non-volatile code and constant storage. Includes a flash accelerator to minimise wait states and support execute-in-place (XIP) operation." },
      { name: "DMA Controller", type: "controller", info: "8-channel DMA controller with programmable priority and burst transfers. Acts as a bus master on the AHB matrix, enabling high-throughput peripheral-to-memory and memory-to-memory transfers without CPU overhead." },
      { name: "Debug Controller", type: "controller", external: true, info: "ARM CoreSight debug subsystem with SWD/JTAG interface, ETM trace, and ITM stimulus ports. Provides full non-intrusive debug with hardware breakpoints, data watchpoints, and real-time trace via TPIU." },
      { name: "GPIO", type: "peripheral", external: true, info: "32-bit GPIO port with alternate function multiplexing, Schmitt-trigger inputs, and configurable drive strength. Directly to FPGA/ASIC I/O pads for board-level connectivity." },
      { name: "UART", type: "peripheral", external: true, info: "Two UART channels with hardware flow control (CTS/RTS), DMA support, and configurable baud rates up to 6 Mbaud. Used for debug console and inter-device serial communication." },
      { name: "SPI", type: "peripheral", info: "SPI master/slave controller with configurable clock polarity/phase, up to 50 MHz clock, and 8-entry TX/RX FIFOs. Supports DMA for high-throughput data streaming to external flash or sensors." },
      { name: "I2C", type: "peripheral", info: "I²C master controller supporting standard (100 kHz), fast (400 kHz), and fast-plus (1 MHz) modes. Includes clock stretching, multi-master arbitration, and 10-bit addressing." },
      { name: "Timer", type: "peripheral", info: "Four 32-bit timer/counter channels with PWM output, input capture, and output compare modes. Supports chained operation for 64-bit timing and DMA trigger generation." },
      { name: "RTC", type: "peripheral", info: "Real-time clock with 32.768 kHz crystal oscillator input, calendar function, and programmable alarm. Operates in low-power standby and generates wake-up interrupts." },
      { name: "Accelerator Slot", type: "interface", info: "Standardised AHB slave interface slot with interrupt and DMA request lines for integrating custom hardware accelerators. Includes a template wrapper with register interface for rapid IP onboarding." },
    ],
  ...
  },
  {
    ...
    blockDiagram: [
      { name: "ARM Cortex-A53", type: "processor", info: "64-bit ARMv8-A application processor with quad-issue in-order pipeline, NEON SIMD, and cryptographic extensions. Configured in single-core mode running at up to 1 GHz for high-throughput application workloads." },
      { name: "AHB Bus Matrix", type: "interconnect", info: "Multi-layer AHB bus matrix enabling concurrent master access. Supports simultaneous transactions from the processor and DMA controller to different slaves with configurable arbitration." },
      { name: "SRAM (128 KB)", type: "memory", info: "128 KB tightly-coupled SRAM for low-latency data access. Used primarily for DMA buffers and performance-critical data structures." },
      { name: "Flash (256 KB)", type: "memory", info: "256 KB embedded flash for bootloader and persistent configuration storage. Includes ECC protection and sector-level erase granularity." },
      { name: "DMA Controller", type: "controller", info: "16-channel DMA controller with scatter-gather support, linked-list descriptors, and per-channel priority. Handles high-bandwidth peripheral servicing with minimal CPU intervention." },
      { name: "Debug Controller", type: "controller", external: true, info: "ARM CoreSight debug and trace subsystem with JTAG/SWD access, ETM program trace, and cross-trigger interface (CTI). Supports multi-core debug synchronisation and real-time trace export." },
      { name: "GPIO", type: "peripheral", external: true, info: "64-bit GPIO with per-pin alternate function selection, interrupt generation, and configurable slew rate. Directly to FPGA/ASIC I/O pads." },
      { name: "UART", type: "peripheral", external: true, info: "Four UART channels with DMA support, hardware flow control, and IrDA encoding. Provides debug console and high-speed serial communication up to 12 Mbaud." },
      { name: "SPI", type: "peripheral", info: "Quad-SPI controller with XIP support for external flash, dual/quad I/O modes, and DMA integration. Supports clock rates up to 100 MHz for high-bandwidth external memory access." },
      { name: "I2C", type: "peripheral", info: "I²C master/slave controller with high-speed mode (3.4 MHz), bus clear recovery, and SMBus/PMBus compatibility. Supports device management for sensor hubs." },
      { name: "Timer", type: "peripheral", info: "Eight 32-bit timer channels with advanced PWM, dead-time insertion, and encoder interface modes. Supports motor control and precision timing applications." },
      { name: "RTC", type: "peripheral", info: "Real-time clock with tamper detection, timestamp capture, and battery-backed operation. Includes calibration register for crystal frequency trimming." },
      { name: "Accelerator Slot", type: "interface", info: "Standardised AHB slave interface with interrupt, DMA request, and coherency signals for custom hardware accelerators. Includes reference wrapper with AXI-to-AHB bridge option." },
    ],
