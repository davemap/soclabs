...
import {
  ...
  ImageIcon, Upload, Loader2, Sigma,
} from "lucide-react";
      ...
      try {
        ...
        if (textarea) {
          insertAtCursor(textarea, `\n![${file.name}](${publicUrl})\n`, (val) =>
            updateSection(index, "body", val)
          );
        } else {
          updateSection(index, "body", sections[index].body + `\n![${file.name}](${publicUrl})\n`);
        }
        ...
        >
          <CardContent className="p-4 space-y-3">
            ...
            <div className="flex items-center gap-0.5 px-1 py-1 rounded-lg border border-border/60 bg-muted/30 flex-wrap">
              <ToolbarButton
                ...
                onClick={() => {
                  ...
                  if (ta) wrapSelection(ta, "**", "**", (v) => updateSection(i, "body", v));
                }}
              />
              <ToolbarButton
                icon={Underline}
                ...
                onClick={() => {
                  const ta = textareaRefs.current[i];
                  if (ta) wrapSelection(ta, "<u>", "</u>", (v) => updateSection(i, "body", v));
                }}
              />
            ...
            <Textarea
              ...
              placeholder="Write content in Markdown... Use $formula$ for inline math, $$formula$$ for block math"
              value={section.body}
            ...
            <p className="text-[10px] text-muted-foreground">
              Supports Markdown, LaTeX formulas ($..$ inline, $$..$$ block), and image uploads. Drag & drop images directly into the editor.
            </p>
          </CardContent>
        </Card>
      ))}
